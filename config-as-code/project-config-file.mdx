...
The template text is what is stored and what `ravion project config pull` exports, so the file round-trips unchanged. `ravion project config apply` orders stack runs so producers run first and rejects reference cycles. For addresses across environments and projects, expressions, and how references resolve, see [output references](/modules/output-references).
