...
"og:image": "https://www.ravion.com/og/docs/guides/multi-region.png"
"twitter:image": "https://www.ravion.com/og/docs/guides/multi-region.png"
---
...
| ---------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **DNS only**                                         | The regional ALB directly                                    | The public hostname, such as `api.example.com` | You want the simplest setup, or you're serving an API or WebSocket traffic that doesn't benefit from a CDN                        |
...
Route 53 alias records need two values per region: the load balancer's **DNS name** and its **canonical hosted zone ID**. Both are stack outputs of the module that owns the ALB.
...
Set **`alias_evaluate_target_health: true`** on every record. With it, Route 53 checks the ALB's target health before answering: if every target behind the `eu-west-1` ALB is unhealthy, Route 53 stops returning that record and European users fail over to `us-east-2` automatically. Without it, a dead region keeps receiving traffic. This works with no extra Route 53 health check, because Route 53 reads ALB target health directly.
...
<Steps>
  ...
  <Step title="Add a CloudFront module with a custom origin">
    ...
    The distribution needs a certificate in `us-east-1` for its alias; request one with a fourth
    `rvn-acm-certificate` instance.

    ```yaml ravion.yaml highlight={7-8,10}
    - givenId: cdn
      name: CDN
      type: rvn-cloudfront
      version: 1.3.1
      input:
        ...
        distribution_aliases:
          - api.example.com
        certificate:
          moduleGivenIdRef: api-cert-use1 # rvn-acm-certificate in us-east-1
    ```
