...
| ----------------------------- | ------------------------------------------------ | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| **Load balancer only**        | The regional ALB directly                        | The public hostname, such as `api.example.com`   | You want the simplest setup, or serve API or WebSocket traffic that doesn't benefit from a CDN         |
...
<Note>
  If a regional target isn't an AWS load balancer (a Cloudflare tunnel, another provider's
  endpoint), use a `CNAME` record instead: `target_type: standard`, the hostname in
  `record_value`, a low `ttl`, and a Route 53 health check in `health_check_id` — CNAMEs can't use
