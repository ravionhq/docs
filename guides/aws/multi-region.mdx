...
| ---------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **Load balancer only**                               | The regional ALB directly (A alias or CNAME)                                  | The public hostname, such as `api.example.com` | You want the simplest setup, or you're serving an API or WebSocket traffic that doesn't benefit from a CDN                        |
...
```yaml ravion.yaml highlight={9,20,62,73}
modules:
  # ---------- us-east-2 ----------
  - givenId: vpc-use2
    name: VPC us-east-2
  ...
  - givenId: api-use2
    ...
    input:
      ...
      image_repository: 123456789012.dkr.ecr.us-east-2.amazonaws.com/my-api
      container_port: 8080
  ...
  - givenId: api-euw1
    ...
    input:
      ...
      host_header_values:
        - api.example.com
      build_source: image_registry
      image_repository: 123456789012.dkr.ecr.eu-west-1.amazonaws.com/my-api
      container_port: 8080
      health_check_path: /healthz
```
...
- **Build once, deploy everywhere.** Don't let each region build its own image. Give every regional `rvn-ecs-web` instance `build_source: image_registry` pointing at an ECR repository in its own region (create one `my-api` repo per region), then use a single [pipeline](/concepts/pipelines) whose `build:image` step pushes the same image to every regional repository and whose `deploy` steps release that one build to each instance:
  ...
  steps:
    - id: build_api
      ...
      destinations:
        - id: ecr_use2
          type: ecr
          repository_arn: arn:aws:ecr:us-east-2:123456789012:repository/my-api
        - id: ecr_euw1
          type: ecr
          repository_arn: arn:aws:ecr:eu-west-1:123456789012:repository/my-api
      infrastructure:
        type: ec2
  ...
  The image is built once and pushed to both registries, so the digest is identical in each region and every service pulls from its local ECR. Make the deploys sequential instead of `parallel` if you want a canary region to finish first.
...
```yaml ravion.yaml highlight={4-7,14-17}
records:
  - type: CNAME
    name: api.example.com
...
Apply the change and skip to [Step 5](#step-5-verify-routing) if you're using the load-balancer-only topology.
...
Compared with load balancer only, you also get TLS termination at the edge, connection reuse to the origin, HTTP/2 and HTTP/3, optional WAF, and [redirect rules](/module-definitions/catalog/rvn-cloudfront) managed in one place instead of per region.
...
- **Parameter Store is regional; Secrets Manager isn't.** A Secrets Manager secret can be replicated to every region you deploy in, so one secret serves all regions — each service's `valueFrom` uses the replica ARN for its own region (same name, different region in the ARN). SSM Parameter Store has no replication: if you use it, create the parameters separately in each region. See [Managing secrets](/guides/managing-secrets).
...
- **Rollouts are only as coordinated as your pipeline.** The build-once pipeline above keeps versions aligned, but there is no atomic multi-region switch: while the deploy steps run, regions briefly serve different versions. Order the deploy steps if one region should act as a canary.
