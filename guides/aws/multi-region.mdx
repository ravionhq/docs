...
```yaml ravion.yaml highlight={9,20,63,74}
modules:
  # ---------- us-east-2 ----------
  - givenId: vpc-use2
    name: VPC us-east-2
...
- **Build once, deploy everywhere.** Don't give each region its own build. Use one [pipeline](/concepts/pipelines) with a single `build` step against one of the instances, then a `deploy` step per region that all take the same `build_ref`, so every region runs the identical image:

  ```yaml pipeline.yaml highlight={12,17}
  steps:
    - id: build_api
      type: build
    ...
    - parallel:
        - id: deploy_use2
          ...
          module_instance: << pipeline.variant.id >>.api-use2
          input:
            image_ref: << steps.build_api.output.build_ref >>
        - id: deploy_euw1
          type: deploy
  ...
  Make the deploys sequential instead of `parallel` if you want a canary region to finish first. The image lives in the build instance's ECR repository; ECS in the other region pulls it cross-region, which works but adds a little pull latency and data-transfer cost. Turn on ECR cross-region replication if that matters to you.
...
- **Rollouts are only as coordinated as your pipeline.** The build-once pipeline above keeps versions aligned, but there is no atomic multi-region switch: while the deploy steps run, regions briefly serve different versions. Use each service's [deploy strategy](/modules/deploy) to keep a bad build from reaching users, and order the deploy steps if one region should act as a canary.
