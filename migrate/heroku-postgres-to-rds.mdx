...
- givenId: db
  ...
  input:
    ...
    db_name: myapp
    username: myapp
    allowed_security_group_ids: []   # the ECS service security groups
```
...
<Steps>
  ...
  <Step title="Restore into RDS">
    ...
    pg_restore --verbose --clean --if-exists --no-acl --no-owner \
      --jobs=4 \
      --dbname="postgresql://myapp:PASSWORD@myapp-production.xxxx.us-east-1.rds.amazonaws.com:5432/myapp?sslmode=require" \
      latest.dump
    ```
...
3. Backfill rows that changed on Heroku between the initial restore and the moment dual writes started (compare `updated_at` columns, or re-copy the affected tables).
