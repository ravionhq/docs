...
Build two modules concurrently, and once both builds succeed, deploy both concurrently. Each `parallel` block completes before the next step starts, so no deploy begins until both builds finish. The build block sets `fail_strategy: cancel-all`, so if one build fails the other is cancelled immediately — the default `finish-all` would let it run to completion before the block fails.
